//! Four-avenue web ↔ API binding for `opto-sync`.
//!
//! 1. Direct read-only DB via `*-lib-core` named queries (no migrations).
//! 2. Stateless HTTP from `app.opto-sync.dev` to `api.opto-sync.dev`.
//! 3. Stateful TLS 1.3/mTLS TCP to `api.opto-sync.dev:7443`.
...
pub const GITHUB_ORG: &str = "opto-sync";
pub const ORG_SLUG: &str = "opto-sync";
pub const DNS_ZONE: &str = "opto-sync.dev";
