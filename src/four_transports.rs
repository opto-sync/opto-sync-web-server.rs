#![forbid(unsafe_code)]
//! The four avenues this web server can use to reach `opto-sync-api-server`.
//!
...
//! to opto-sync — the environment prefix, the service slug that names its NATS
//! subjects, and the api route that accepts an envelope.
//!
//! | # | Mode | What happens |
//! |---|------|--------------|
//! | 1 | `direct_read` | this server queries Postgres itself through `opto-sync-lib-core`, read-only |
//! | 2 | `http` | stateless request to the `opto-sync-api-server` cluster |
//! | 3 | `tcp` | framed, held-open connection to the `opto-sync-api-server` cluster |
...
//! `opto-sync-lib-core`'s read context, and hand it to [`gateway_from_env`]. See
...
///
/// `OPTO_SYNC_API_URL`, `OPTO_SYNC_NATS_URL`, and the rest; see the
